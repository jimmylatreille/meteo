...
class Meteo {
	...
	public $temp;
	const LOCATION_ID = '3534';
	const TIME_ZONE   = 'America/Montreal';

	public function __construct(){
		
		date_default_timezone_set(self::TIME_ZONE);
		$weather = simplexml_load_file("http://weather.yahooapis.com/forecastrss?w=".self::LOCATION_ID."&u=c");
		preg_match('/\w*\,\s{1}\d{2}|\d{1}\s{1}/', (string)$weather->channel->item->description, $match, PREG_OFFSET_CAPTURE);
		$this->temp = explode(",", strtolower(preg_replace('/\s/', '', $match[0][0])));
	}
